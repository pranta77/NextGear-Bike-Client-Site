...
export default function SignIn() {
  ...
  const handleLogin = (e) => {
    ...
    e.preventDefault();
    // if (loginUser.email && loginUser.password)
    signInUser(loginUser.email, loginUser.password, navigate);
  };
  return (
    <div>
      ...
      <div className="border p-8 rounded-md bg-teal-950 absolute max-sm:bottom-1 xl:top-72 max-sm:left-52 max-sm:w-full left-72 transform -translate-x-1/2 -translate-y-1/2 text-orange-300 ">
        ...
        {!loading && (
          <form onSubmit={handleLogin}>
            ...
            <br />
            <NavLink to={"/signup"}>
              <button className="btn btn-link">NEW USER? PLEASE SIGNUP</button>
            </NavLink>
          </form>
        )}
        ...
        {user?.email && (
          <div role="alert" className="alert alert-success">
            ...
            <span>SignIn has been confirmed!</span>
          </div>
        )}
        {error && (
